...
abstract class PmdInvoker {
    static void invoke(Pmd pmdTask) {
        ...
        ScopedSystemProperties.newPropertyEnvironment().withCloseable { environment ->
            environment.setProperty("java.class.path", pmdClasspath.files.join(File.pathSeparator))
    ...
    static VersionNumber determinePmdVersion(ClassLoader antLoader) {
        Class pmdVersion
        try {
